...
#[derive(Debug, Serialize, Deserialize, Dummy, Clone, PartialEq, Eq, sqlx::Type)]
...
pub struct UserStat {
    ...
    name: String,
    gender: Gender,
    #[dummy(faker = "DateTimeBetween(before(5 * 365), before(90))")]
...
async fn main() -> Result<()> {
    ...
    for i in 1..=5000 {
        ...
        println!("Raw {} inserted in {:?}", i, start.elapsed());
    }
    Ok(())
}

fn to_string_list(list: Vec<i32>) -> String {
    format!("ARRAY{:?}", list)
}
...
async fn raw_insert(users: HashSet<UserStat>, pool: &PgPool) -> Result<()> {
    ...
    for user in users {
        sql.push_str(&format!(
            ...
            to_string_list(user.recent_watched),
            to_string_list(user.viewed_but_not_started),
            to_string_list(user.started_but_not_finished),
            to_string_list(user.finished),
...
// bulk insert 和 raw insert 插入的速度是差不多的
